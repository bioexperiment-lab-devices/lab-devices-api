...
from lab_devices.discovery import _probe_port, discover_devices
...
class TestProbePort:
    async def test_probe_finds_pump(self) -> None:
        mock_conn = AsyncMock()
        mock_conn.send_and_receive = AsyncMock(return_value=bytes([0x0A, 0x00, 0x00, 0x00]))
        with patch("lab_devices.discovery.SerialConnection", return_value=mock_conn):
    ...
    async def test_probe_finds_nothing(self) -> None:
        mock_conn = AsyncMock()
        mock_conn.send_and_receive = AsyncMock(side_effect=DeviceTimeoutError("timeout"))
        with patch("lab_devices.discovery.SerialConnection", return_value=mock_conn):
    ...
    async def test_probe_connection_error(self) -> None:
        mock_conn = AsyncMock()
        mock_conn.connect = AsyncMock(side_effect=DeviceConnectionError("cannot open"))
        with patch("lab_devices.discovery.SerialConnection", return_value=mock_conn):
...
class TestDiscoverDevices:
    ...
    async def test_discover_with_extra_ports(self) -> None:
        ...
        with (
            ...
            result = await discover_devices(extra_ports=["/dev/ttyUSB0"])
        assert len(result) == 0
