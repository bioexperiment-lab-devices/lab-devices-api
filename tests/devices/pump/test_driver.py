...
class TestPumpDriver:
    async def test_start_rotation_left(self) -> None:
        ...
        conn.send_command.assert_awaited_once_with(bytes([0x0B, 0x6F, 0x00, 0x05, 0x00]))

    async def test_start_rotation_right(self) -> None:
        pump, conn = make_pump()
        await pump.start_rotation(speed=10, direction=Direction.RIGHT)
        conn.send_command.assert_awaited_once_with(bytes([0x0C, 0x6F, 0x00, 0x0A, 0x00]))
    ...
    async def test_stop_rotation(self) -> None:
        ...
        conn.send_command.assert_awaited_with(bytes([0x0B, 0x6F, 0x00, 0x00, 0x00]))
    ...
    async def test_set_rotation_speed(self) -> None:
        ...
        conn.send_command.assert_awaited_once_with(bytes([0x0A, 0x00, 0x00, 0x03, 0x00]))
    ...
    async def test_pour_volume_left(self) -> None:
        ...
        conn.send_command.assert_awaited_once_with(bytes([0x10, 0x00, 0x00, 0x00, 0x32]))

    async def test_pour_volume_right(self) -> None:
        pump, conn = make_pump()
        await pump.pour_volume(direction=Direction.RIGHT, volume=100)
        conn.send_command.assert_awaited_once_with(bytes([0x11, 0x00, 0x00, 0x00, 0x64]))
